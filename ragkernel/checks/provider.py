...
def _reachable_via_proxy(base: str, host: str, proxy: str) -> CheckResult:
    ...
    try:
        ...
        pass  # 拿到 HTTP 响应（4xx/5xx 也算）就说明连通
    except (urllib.error.URLError, OSError) as e:
        return failed("provider.network", CATEGORY, "provider · network",
                      f"经代理 {proxy} 无法到达 {host}（{getattr(e, 'reason', e)}）",
                      fix=FIX, host=host, proxy_used=True, proxy=proxy)
    ms = int((monotonic() - t0) * 1000)
    return passed("provider.network", CATEGORY, "provider · network",
                  f"{host} 经代理可达（{ms}ms）", host=host, proxy_used=True, proxy=proxy)


# ------------------------------------------------------------------ auth（零成本，绝不计费）

class _NoZeroCostEndpoint(Exception):
    """该 provider 没有可用于零成本鉴权验证的端点。"""


class ProviderProbe:
    """让 auth 检查不必知道各家 provider 的端点细节。子类只实现 _probe()。"""
    ...
    def check_auth(self) -> CheckResult:
        try:
            status = self._probe()
        except _NoZeroCostEndpoint:
            return skipped("provider.auth", CATEGORY, "provider · auth",
                           "该 provider 无零成本鉴权端点，未验证（避免计费）；"
                           "`ragkernel setup` 会做一次带最小推理的完整验证", verified=False)
        except (urllib.error.URLError, OSError) as e:
...
class OpenAICompatibleProbe(ProviderProbe):
    def _probe(self) -> int:
        ...
        status = self._get_status(f"{self.base}/models", headers)
        if status == 404:
            raise _NoZeroCostEndpoint
        return status


class AnthropicProbe(ProviderProbe):
    def _probe(self) -> int:
        # Anthropic 官方有免费的 GET /v1/models；MiniMax 的兼容端点未必有 → 404 则跳过
        headers = {"x-api-key": self.key, "anthropic-version": "2023-06-01"}
        status = self._get_status(f"{self.base}/v1/models", headers)
        if status == 404:
            raise _NoZeroCostEndpoint
        return status

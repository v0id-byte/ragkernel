...
# 堆叠式故障条目的起始键（非表格形式：故障代码：E-42 / 原因：… / 处理：…）
_FAULTKEY = re.compile(r"^\s*(?:故障代码|故障码|报警代码|报警码|错误代码|故障代号)\s*[：:]")
...
def md_to_blocks(text: str) -> list[Block]:
    ...
    def flush_prose():
        ...
        if not para:
            return
        nonblank = [l.strip() for l in para.splitlines() if l.strip()]
        kv = [l for l in nonblank if _KV.match(l) or _KV_DASH.match(l)]
        if nonblank and len(kv) >= max(1, round(len(nonblank) * 0.6)):
            j = 0
            while j < len(nonblank):
                line = nonblank[j]
                if _FAULTKEY.match(line):
                    # 堆叠式故障条目：代码行 + 后续原因/处理并成一片（否则处理方法与故障码分家）
                    grp = [line]
                    j += 1
                    while j < len(nonblank) and not _FAULTKEY.match(nonblank[j]) and (
                        _KV.match(nonblank[j]) or _KV_DASH.match(nonblank[j])
                    ):
                        grp.append(nonblank[j])
                        j += 1
                    blocks.append(Block("kv", "\n".join(grp), section_path=_sp(section)))
                else:
                    blocks.append(Block("kv", line, section_path=_sp(section)))  # 其余键值逐行（针脚等）
                    j += 1
        else:

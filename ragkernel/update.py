...
_UNIT_CANDIDATES = ("ragkernel.service", "ragkernel-web.service")


def managed_unit() -> str | None:
    """管理本机 ragkernel 的 systemd 单元名；没有则 None。

    `runtime_mode()` 回答的是「**我这个进程**怎么跑起来的」，对服务进程是对的；但
    `ragkernel upgrade` 通常是运维在交互 shell 里敲的，那个进程不由 systemd 拉起、
    没有 INVOCATION_ID，会被判成 process 模式。若就此告诉用户「手动重启即可」，
    真正在跑旧代码的那个**服务**进程没人动——它会一直用内存里的旧代码提供服务。

    所以「我是不是被管理的那个进程」和「这台机器上有没有被管理的服务」是两个问题，
    必须分开问。
    """
    ...
    for unit in _UNIT_CANDIDATES:
        try:
            ...
            return None   # 没有 systemctl（容器 / 非 systemd 发行版）
        if r.stdout.strip() == "active":
            return unit
    return None


def restart_hint(mode: str | None = None) -> tuple[str, str]:
    """(谁负责重启, 给用户的命令)。命令由服务端给全，前端与 CLI 都不拼。"""
    mode = mode or runtime_mode()
    if mode == "systemd":
        return "systemd", ""            # 本进程就是那个单元，退出即被拉起
    unit = managed_unit()
    if unit:
        return "manual-service", f"sudo systemctl restart {unit}"
    return "manual", "ragkernel serve"
...
def _fetch(endpoint: str, etag: str | None) -> tuple[dict | None, str | None]:
    ...
    try:
        with urllib.request.urlopen(req, timeout=_TIMEOUT) as r:
            ...
            # 合法 JSON 但不是对象（配错地址指到某个数组接口、CDN 返回错误页的 JSON 包装）
            # 会让下游 .get() 抛 AttributeError 逃出 check()，把「非致命的版本检查」变成
            # `ragkernel update` / `version` 直接崩溃。企业自建 mirror 配错是可预期的。
            if not isinstance(data, dict):
                raise UpdateError(f"版本清单不是 JSON 对象（拿到 {type(data).__name__}）")
            return data, r.headers.get("ETag") or etag
...
def check(force: bool = False) -> UpdateStatus:
    ...
    if not isinstance(manifest, dict):   # 缓存被手改坏或旧格式残留
        manifest = {}
    etag = cache.get("etag")
    ...
    try:
        ...
        # 只读安装目录 / 只读容器里 .ragkernel 不可写。缓存写不下去只是「下次还得重查」，
        # 不该让 `ragkernel update` 或 doctor 整个崩掉——版本检查全程是非致命的。
        error = error or f"缓存写入失败：{type(e).__name__}: {e}"
    return _status(manifest, channel, endpoint, checked_at, error)
...
def _install_python() -> str:
    ...
    比「当前解释器版本」更准：install.sh 用 `uv sync --python <ver>`，那个版本记在
...
def _python_blocker(spec: str | None) -> dict | None:
    ...
    try:
        ...
        # packaging 是传递依赖而非直接依赖。缺了就跳过这一层——installer 与 pyproject 的
        # requires-python 仍会兜底拦住，只是归因没这么早、这么清楚。
        return None

    target = _install_python()
    try:
        # install.json 里常是 "3.12" 这种两段版本，补足成可比较的三段
        ver = Version(target if target.count(".") >= 2 else target + ".0")
        if ver in SpecifierSet(spec):
            return None
    except Exception:  # noqa: BLE001 —— 版本串或 specifier 不合法时不拦，交给 installer
        return None
    return {"type": "python", "current": target, "required": spec,
            "message": f"目标 Python {target} 不满足新版要求 {spec}"}
...
class UpdateController:
    ...
    def apply(self, target: str, *, to_version: str | None = None,
        ...
        handled_by, restart_cmd = restart_hint(mode)
        return {"update_id": update_id, "mode": mode,
                "restart_required": True,
                "restart_handled_by": handled_by,
...
def summary() -> dict:
    ...
    **只读缓存，绝不联网。** 这三个调用方都要求即时返回：Web 端点卡 8 秒超时不可接受，
    ...
    """
    mode = runtime_mode()
    # 只问一次：restart_hint 在非 systemd 形态下会起 systemctl 子进程探单元，
    # 调两次就是每次 summary 多花两轮进程创建（这是要挂到 Web 端点上的）。
    handled_by, restart_cmd = restart_hint(mode)
    st = cached_status()
    state = read_state()
    ...
    return {
        ...
        "runtime": {"mode": mode, "can_self_update": can_self_update(mode),
                    ...
                    # 「我是不是被管理的那个进程」与「机器上有没有被管理的服务」是两个问题
                    "restart_handled_by": handled_by,
                    "restart_command": restart_cmd},
